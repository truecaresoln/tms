...
def get_data(filters):
	...
	data = []	
	if viatris == 1:
		data1 = frappe.db.sql("""select distinct `tabProject`.company,tpd.deliverable,`tabProject`.turacoz_start_date,`tabProject`.name,`tabProject`.project_title,date(`tabProject`.creation) as creation,
	...
	for rec in data1:
		...
		if data5:
			...
			row["po_amount"] = "Sales Order not generated"
			row["po_amount_inr"] = "Sales Order not generated"
			row["pending_unraised_inr"]	= "Sales Order not generated"
			row["pending_unraised"]	= 'Sales Order not generated'				
