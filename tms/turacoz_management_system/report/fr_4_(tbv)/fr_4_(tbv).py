...
def get_data(filters):
	...
	data=[]
	data4 = frappe.db.sql("""select payment_date,sum(net_total) as 'payment_received_in_bank',sum(igst) as 'gst',(sum(net_total)*0.10) as 'tds',(sum(net_total)+sum(igst))-(sum(net_total)*0.10) as 'amount_received'  from `tabPayment Request` tpr where company='Turacoz B.V.' and customer_name not in ('Viatris Centre of Excellence','Mylan Centre of Excellence') and payment_date>='{0}' and payment_status in ('Paid','Partly Paid') and status not in ('Cancelled') group by payment_date order by 1 desc
	...
	while start_date1<last_date1:
		...
		data2 = frappe.db.sql("""select MONTH(invoice_date),MONTHNAME(invoice_date) as 'month',sum(grand_total*conversion_rate) as 'received_amount'  from `tabPayment Request` tpr where company='Turacoz B.V.' and customer_name not in ('Viatris Centre of Excellence','Mylan Centre of Excellence')
		...
		data3 = frappe.db.sql("""select MONTH(date_of_transaction),YEAR(date_of_transaction),sum(transaction_amount) from `tabBank Transaction Details`
				where company='Turacoz B.V.' and customer not in ('Viatris Centre of Excellence','Mylan Centre of Excellence') and MONTH(date_of_transaction)='{0}' and YEAR(date_of_transaction)='{1}' group by MONTH(date_of_transaction),YEAR(date_of_transaction) order by 1
			""".format(month,year))
	...
	row['received_amount'] = "<div><span><center><b>VAT (B = 21% of A)</b></center></span></div>"
