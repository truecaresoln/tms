...
def get_data():
	...
	mm_name = todays_date.strftime("%m")
 ...
 # 		and tp.project_current_status not in ('Cancelled') GROUP by tso.currency""".format(month_name,year), as_dict=True)
	...
	row5 = {}
	row5["particular"] = "Total Payment Due Till " + "("+ month_name + " " + str(year) +") - J"
	...
	dataExpected = frappe.db.sql("""select sum(tpr.grand_total) as total_amount,
		...
		WHERE tpr.payment_status not in ('Cancelled','Paid') and tpr.due_date is not NULL and
		...
		MONTH(tpr.due_date) <= '{0}' and YEAR(tpr.due_date) <= '{1}' GROUP by tpr.invoice_currency""".format(mm_name,year), as_dict=True)
	...
	dataExpectedTHS = frappe.db.sql("""select sum(tpr.grand_total) as total_amount,
		...
		from `tabPayment Request` tpr 
		left join `tabCurrency Exchange Rate` ter on tpr.invoice_currency = ter.name
		WHERE tpr.payment_status not in ('Cancelled','Paid') and tpr.due_date is not NULL 
	...
	final_total_amount_inr_expectedTSPL = float()	
	for recExpected in dataExpected:
		final_total_amount_inr_expectedTSPL += float(recExpected.total_inr)
		final_total_amount_inr_expectedTSPL = round(final_total_amount_inr_expectedTSPL, 2)	
	row5["tspl"] = final_total_amount_inr_expectedTSPL
	...
	final_total_amount_inr_expectedTBV = float()	
	for recExpected in dataExpected:
		final_total_amount_inr_expectedTBV += float(recExpected.total_inr)
		final_total_amount_inr_expectedTBV = round(final_total_amount_inr_expectedTBV, 2)	
	row5["tbv"] = final_total_amount_inr_expectedTBV
	...
	final_total_amount_inr_expectedViatris = float()	
	if dataViatrisPending:
		final_total_amount_inr_expectedViatris += float(dataViatrisPending[0]['total_pending_amount'])
		final_total_amount_inr_expectedViatris = round(final_total_amount_inr_expectedViatris, 2)	
		row5["viatris"] = final_total_amount_inr_expectedViatris
	else:
		row5["viatris"] = 0.0	
	...
	row6 = {}
	row6["particular"] = "Payment Received " + "("+ month_name + " " + str(year) +") - K"
	...
	dataReceivedViatris = frappe.db.sql("""select sum(tpr.grand_total) as total_amount, 
		...
		MONTH(tpr.payment_date) = '{0}' and YEAR(tpr.payment_date) = '{1}' GROUP by tpr.invoice_currency""".format(mm_name,year), as_dict=True)
	final_total_amount_inr_receivedViatris = float()	
	if dataReceivedViatris:
		final_total_amount_inr_receivedViatris = float(dataReceivedViatris[0]['total_inr'])
		final_total_amount_inr_receivedViatris = round(final_total_amount_inr_receivedViatris, 2)	
		row6["viatris"] = final_total_amount_inr_receivedViatris
	else:
		row6["viatris"] = 0.0	
...
def get_columns():
	cols = [
			...
			},
			{
				...
				"label": _("Total Amount(Excluding Viatris)"),
				"fieldtype": "Currency",
				"width": "150"
			},
			{
