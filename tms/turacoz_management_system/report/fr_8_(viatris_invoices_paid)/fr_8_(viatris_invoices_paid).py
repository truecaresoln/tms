...
def get_data(filters):
	...
	data=[]
	data1 = frappe.db.sql("""select tpr.name, tpr.invoice_date, tpr.project,tip.ion, ROUND(tpr.net_total,2) as 'net_total',ROUND(tpr.total_taxes,2) as 'total_taxes',ROUND((tpr.net_total*0.1),2) as 'tds', ROUND(tpr.grand_total,2) as 'grand_total',ROUND((tpr.net_total+tpr.total_taxes)-(tpr.net_total*0.1)) as 'receivable', tpr.currency as 'Currency',tpr.due_date  as 'Due Date', tpr.payment_date as 'Payment Date',tpe.reference_no, tpe.reference_date
			from `tabPayment Request` tpr
			left join `tabInvoice Particulars` tip on tpr.name = tip.parent
			left join `tabPayment Entry` tpe on tpr.project = tpe.project and tpr.name=tpe.payment_request 
			where tpr.customer_name = 'Viatris Centre of Excellence' and tpr.payment_status in ('Paid') and tpr.status not in ('Cancelled') and tpr.invoice_date between '{0}' and '{1}' group by tpr.name""".format(start_date,end_date),as_dict=True)
	total_net=0.0
	total_taxes=0.0
	total_tds=0.0
	total_grand=0.0
	...
	row={}
	row["name"] = "<div><span><b>Viatris Paid Invoices From " + str(start_date) + " To " + str(end_date)+"</b></span></div>"
	data.append(row)
...
def get_columns(filters):
			...
			},
			{
				...
				"label": _("Amount Received After Deducting TDS (E = D-C)"),
				"fieldtype": "Data",
				"width": "200"
			},
			{
